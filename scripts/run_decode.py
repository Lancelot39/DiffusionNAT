...
if __name__ == '__main__':
    ...
    parser.add_argument('--step', type=int, default=200, help='if less than diffusion training steps, like 1000, use ddim sampling')
    ...
    for lst in glob.glob(args.model_dir):
        ...
        for checkpoint_one in checkpoints:
            if '80000' not in checkpoint_one:
                continue
            for seed in [1000]:
                # COMMAND = f'python -m torch.distributed.launch --nproc_per_node=8 --master_port={22233 + int(args.seed)} --use_env sample_seq2seq_ddp.py ' \
