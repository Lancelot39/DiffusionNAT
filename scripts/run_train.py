...
if __name__ == '__main__':
    ...
    parser.add_argument('--src_seq_len', type=int, default=512, help='max len of input sequence')
    ...
    parser.add_argument('--config_name', type=str, default='facebook/bart-base', help='config of pre-trained models')
    parser.add_argument('--vocab', type=str, default='bart', help='use bert vocab or load external vocab dict if given as path')
    parser.add_argument('--use_plm_init', type=str, default='bart', choices=['no', 'bert'], help='load init parameter from the pre-trained lm')
    ...
    COMMANDLINE = f" OPENAI_LOGDIR={Model_FILE}  " \
                  ...
                  f"--seq_len {args.seq_len} --src_seq_len {args.src_seq_len} --hidden_t_dim {args.hidden_t_dim} --seed {args.seed} " \
