...
import io
from diffuseq.gaussian_diffusion import index_to_log_onehot, log_onehot_to_index
...
def sample_time(b, num_timesteps, device):
    t = th.randint(0, num_timesteps, (b,), device=device).long()
    pt = th.ones_like(t).float() / num_timesteps
    return t, pt
...
class TrainLoop:
    ...
    def forward_only(self, cond):
        with th.no_grad():
            ...
            batch_size = cond['input_ids'].size(0)
            for i in range(0, batch_size, self.microbatch):
                ...
                t, weights = sample_time(micro_cond['decoder_input_ids'].size(0),
                                         th.tensor(self.diffusion.time_step).to(th.int),
                                         micro_cond['decoder_input_ids'].device)
                log_x_start = index_to_log_onehot(micro_cond['decoder_input_ids'],
                                                  self.diffusion.num_classes)  # [b, v, len]
                log_xt = self.diffusion.q_sample(log_x_start=log_x_start, t=t)
                xt = log_onehot_to_index(log_xt)
                mask_tokens = xt

                self_cond_inputs = None
                if random.random() < 0.5:
                    with torch.no_grad():
                        outputs = self.ddp_model(mask_tokens, micro_cond['input_ids'], micro_cond['input_mask'])
                ...
                if last_batch or not self.use_ddp:
                    loss = F.cross_entropy(logits.reshape(-1, logits.shape[-1]),
                                           micro_cond['decoder_input_ids'].flatten(),
                                           ignore_index=-1, reduce=False)
                    loss = loss.reshape(micro_cond['decoder_input_ids'].size(0), -1)
                    losses['loss'] = th.mean(loss, -1)
                else:
    ...
    def forward_backward(self, cond):
        zero_grad(self.model_params)
        batch_size = cond['decoder_input_ids'].size(0)
        for i in range(0, batch_size, self.microbatch):
            micro_cond = {
                k: v[i: i + self.microbatch].to(dist_util.dev())
                for k, v in cond.items()
            ...
            outputs = self.ddp_model(mask_tokens, micro_cond['input_ids'], micro_cond['input_mask'], self_cond_inputs)
            logits = outputs

            losses = {}
            if last_batch or not self.use_ddp:
                loss = F.cross_entropy(logits.reshape(-1, logits.shape[-1]), micro_cond['decoder_input_ids'].flatten(),
                                       ignore_index=-1, reduce=False)
                loss = loss.reshape(micro_cond['decoder_input_ids'].size(0), -1)
                losses['loss'] = th.mean(loss, -1)
            else:
                with self.ddp_model.no_sync():
                    loss = F.cross_entropy(logits.reshape(-1, logits.shape[-1]),
                                           micro_cond['decoder_input_ids'].flatten(), ignore_index=-1, reduce=False)
                    loss = loss.reshape(micro_cond['decoder_input_ids'].size(0), -1)
                    losses['loss'] = th.mean(loss, -1)
...
def log_loss_dict(diffusion, ts, losses):
    for key, values in losses.items():
        ...
        for sub_t, sub_loss in zip(ts.cpu().numpy(), values.detach().cpu().numpy()):
            quartile = int(4 * sub_t / diffusion.time_step)
